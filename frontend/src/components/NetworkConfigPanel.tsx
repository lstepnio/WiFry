...
type ProfileDialogState =
  | { type: 'save' }
  | { type: 'set-boot'; profile: NetworkProfile }
  | { type: 'delete'; profile: NetworkProfile }
  | { type: 'reset-defaults' }
  | null;
...
export default function NetworkConfigPanel() {
  ...
  const { data: config, loading: loadingConfig, error: configError, refresh } = useApi<FullNetworkConfig>(configFetcher);
  ...
  const { data: profiles, loading: loadingProfiles, error: profilesError, refresh: refreshProfiles } = useApi<NetworkProfile[]>(profilesFetcher);

  const [ap, setAp] = useState<WifiApConfig | null>(null);
  const [eth, setEth] = useState<EthernetConfig | null>(null);
  const [saving, setSaving] = useState(false);
  const [pendingProfileAction, setPendingProfileAction] = useState<ProfileDialogState>(null);
  ...
  const applyConfig = async () => {
    ...
    try {
      ...
      notify('Configuration applied successfully', 'success');
      refresh();
    } catch {
      notify('Failed to apply configuration.', 'error');
    } finally {
      setSaving(false);
    }
  };
  ...
  const resetDefaults = async () => {
    try {
      const res = await fetch('/api/v1/network-config/reset-defaults', { method: 'POST' });
      if (!res.ok) throw new Error('Failed to reset defaults');

      refresh();
      notify('Reset to defaults', 'success');
    } catch {
      notify('Failed to reset defaults.', 'error');
    } finally {
  ...
  const applyProfile = async (id: string) => {
    setWorkingProfileId(id);
    try {
      const res = await fetch(`/api/v1/network-config/profiles/${id}/apply`, { method: 'POST' });
      if (!res.ok) throw new Error('Failed to apply profile');

      refresh();
      notify('Profile applied', 'success');
    } catch {
  ...
  const setBootProfile = async (profile: NetworkProfile) => {
    setWorkingProfileId(profile.id);
    try {
      const res = await fetch(`/api/v1/network-config/profiles/${profile.id}/set-boot`, { method: 'POST' });
      if (!res.ok) throw new Error('Failed to set boot profile');

      refreshProfiles();
      notify('Boot profile set', 'success');
    } catch {
      notify('Failed to set boot profile.', 'error');
    } finally {
      setWorkingProfileId(null);
      setPendingProfileAction(null);
    }
  ...
  const clearBootProfile = async () => {
    setWorkingProfileId('clear-boot');
    try {
      const res = await fetch('/api/v1/network-config/profiles/clear-boot', { method: 'POST' });
      if (!res.ok) throw new Error('Failed to clear boot profile');

      refreshProfiles();
      notify('Boot profile cleared', 'success');
    } catch {
      notify('Failed to clear boot profile.', 'error');
    } finally {
      setWorkingProfileId(null);
    }
  };

  const deleteProfile = async (profile: NetworkProfile) => {
    setWorkingProfileId(profile.id);
    try {
      const res = await fetch(`/api/v1/network-config/profiles/${profile.id}`, { method: 'DELETE' });
      if (!res.ok) throw new Error('Failed to delete profile');

      refreshProfiles();
      notify('Profile deleted', 'success');
    } catch {
      notify('Failed to delete profile.', 'error');
    } finally {
      setWorkingProfileId(null);
      setPendingProfileAction(null);
    }
  ...
  if (loadingConfig) {
    return <PanelState title="Loading Network Config" message="Fetching WiFi and Ethernet settings." variant="loading" />;
  }

  if (configError || !config) {
    return <PanelState title="Network Config Unavailable" message="Unable to load network settings right now." variant="error" />;
  }

  if (!ap || !eth) {
    return <PanelState title="No Network Config Found" message="WiFry did not return editable network settings." variant="empty" />;
  }
    ...
    <>
      <div className="space-y-4">
        ...
        <div className="rounded-lg border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-700 dark:bg-gray-900">
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
            ...
            <div>
              ...
              <select value={ap.channel_width || 20} onChange={e => setAp({...ap, channel_width: parseInt(e.target.value, 10)})}
                className="w-full rounded border border-gray-300 bg-white px-3 py-1.5 text-sm dark:border-gray-600 dark:bg-gray-800 dark:text-white"
            ...
            <Input label="Channel" value={ap.channel} onChange={v => setAp({...ap, channel: parseInt(v, 10) || 0})} type="number" title="0 = auto, 1-11 for 2.4GHz, 36-165 for 5GHz" />
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <button onClick={() => setPendingProfileAction({ type: 'save' })}
          ...
          <button onClick={() => setPendingProfileAction({ type: 'reset-defaults' })}
        ...
        {loadingProfiles ? (
          <PanelState title="Loading Profiles" message="Fetching saved network profiles." variant="loading" />
        ) : profilesError ? (
          <PanelState title="Profiles Unavailable" message="Unable to load saved network profiles right now." variant="error" />
        ) : (profiles ?? []).length > 0 ? (
          <div className="rounded-lg border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-700 dark:bg-gray-900">
            <h3 className="mb-2 text-sm font-medium text-gray-500">Saved Network Profiles</h3>
            <div className="space-y-1">
              {(profiles ?? []).map(profile => (
                <div key={profile.id} className="flex items-center justify-between rounded border border-gray-700 bg-gray-800/50 px-3 py-2">
                  <div className="flex items-center gap-2">
                    <span className="text-xs font-medium text-gray-300">{profile.name}</span>
                    {profile.is_boot_profile && <span className="rounded bg-yellow-900 px-1.5 py-0.5 text-[9px] text-yellow-300">boot profile</span>}
                    {profile.description && <span className="text-xs text-gray-500">{profile.description}</span>}
                  </div>
                  <div className="flex items-center gap-1">
                    <button onClick={() => applyProfile(profile.id)} disabled={workingProfileId === profile.id}
                      className="rounded bg-blue-600 px-2 py-1 text-[10px] text-white hover:bg-blue-700 disabled:opacity-50">
                      {workingProfileId === profile.id ? 'Applying...' : 'Apply'}
                    </button>
                    {!profile.is_boot_profile ? (
                      <button onClick={() => setPendingProfileAction({ type: 'set-boot', profile })} title="Load this profile automatically on boot"
                        className="rounded border border-gray-600 px-2 py-1 text-[10px] text-gray-500 hover:bg-gray-700">
                        Set Boot
                      </button>
                    ) : (
                      <button onClick={clearBootProfile} disabled={workingProfileId === 'clear-boot'}
                        className="rounded border border-yellow-700 px-2 py-1 text-[10px] text-yellow-400 hover:bg-yellow-950 disabled:opacity-50">
                        {workingProfileId === 'clear-boot' ? 'Updating...' : 'Unset Boot'}
                      </button>
                    )}
                    <button onClick={() => setPendingProfileAction({ type: 'delete', profile })}
                      className="rounded bg-red-900 px-2 py-1 text-[10px] text-red-300 hover:bg-red-800">
                      Delete
                    </button>
                  </div>
                </div>
          ...
          <PanelState title="No Saved Profiles" message="Save a network profile after you configure the hotspot and uplink if you want to reuse it later." variant="empty" />
        )}
      </div>

      <ModalDialog
        open={pendingProfileAction?.type === 'save'}
        title="Save Network Profile"
      ...
      <ModalDialog
        open={pendingProfileAction?.type === 'reset-defaults'}
        title="Reset Defaults"
        description='Reset to factory defaults? WiFi SSID will be "WiFry" with password "wifry1234".'
        confirmLabel="Reset defaults"
        confirmTone="danger"
        onCancel={() => setPendingProfileAction(null)}
        onConfirm={resetDefaults}
      />

      <ModalDialog
        open={pendingProfileAction?.type === 'set-boot'}
        title="Set Boot Profile"
        description="Set as boot profile? If WiFi settings are wrong, use fallback IP (169.254.42.1) to recover."
        confirmLabel="Set boot profile"
        onCancel={() => setPendingProfileAction(null)}
        onConfirm={pendingProfileAction?.type === 'set-boot' ? () => setBootProfile(pendingProfileAction.profile) : undefined}
      />

      <ModalDialog
        open={pendingProfileAction?.type === 'delete'}
        title="Delete Profile"
        description={pendingProfileAction?.type === 'delete' ? `Delete profile "${pendingProfileAction.profile.name}"?` : ''}
        confirmLabel="Delete profile"
        confirmTone="danger"
        onCancel={() => setPendingProfileAction(null)}
        onConfirm={pendingProfileAction?.type === 'delete' ? () => deleteProfile(pendingProfileAction.profile) : undefined}
      />
    </>
  );
}
